...
class TelegramRouter
{
    ...
    public function handle()
    {
        ...
        if (!is_object($data) || json_last_error() !== JSON_ERROR_NONE) { $exception = new \ReyhanTeam\TelegramBotRouter\Exceptions\InvalidTelegramUpdateException('Invalid JSON received from Telegram.'); $this->handleException($exception, ['source' => 'router']); return response()->json(['error' => 'Invalid Telegram update'], 400); }
        if (!isset($data->update_id)) { $exception = new \ReyhanTeam\TelegramBotRouter\Exceptions\InvalidTelegramUpdateException('Telegram update_id is missing.'); $this->handleException($exception, ['source' => 'router']); return response()->json(['error' => 'Invalid Telegram update'], 400); }
        $this->update = new TelegramUpdate($data); TelegramBot::setApplication(app()); event(new UpdateReceived($this->update));
        if (isset($this->update->message)) { event(new MessageReceived($this->update)); if (isset($this->update->message->text) && str_starts_with(trim((string) $this->update->message->text), '/')) event(new CommandReceived($this->update)); }
    ...
    public function handleException(Throwable $exception, array $context = []): void { $handlerClass = config('telegram-bot-router.exceptions.handler', TelegramExceptionHandler::class); app()->make($handlerClass)->handle($exception, $context); }
    ...
    public function dispatch(TelegramUpdate $update): void
    {
        ...
        if ($conversationManager->active($update)) { $this->execute(['callback' => fn(TelegramUpdate $update) => $conversationManager->handle($update), 'pattern' => 'conversation', 'middleware' => [], 'constraints' => [], 'rate_limits' => [], 'queue' => ['enabled' => false]], $update); return; }
        $routes = TelegramBot::getRoutes();
        $cachedIndex = TelegramRouteCache::getExactRouteIndexForUpdate($update);
        if ($cachedIndex !== null && isset($routes[$cachedIndex])) { $matchedRoute = $routes[$cachedIndex]; $this->setMatchState($matchedRoute, $update); event(new RouteMatched($update, $matchedRoute)); $this->execute($matchedRoute, $update); return; }
    ...
    {
        if (!isset($update->message->text)) return false; $text = trim((string) $update->message->text); if ($text === '' || !str_starts_with($text, '/')) return false; $parts = preg_split('/\s+/', $text, 2); $command = $parts[0] ?? ''; if (str_contains($command, '@')) $command = explode('@', $command, 2)[0]; return in_array($command, TelegramBot::getConversationCancelCommands(), true);
    }
    ...
    protected function routeMatchScore(array $route, TelegramUpdate $update): int
    {
        ...
        switch ($type) {
            ...
            case 'text':
                if (!isset($update->message->text)) return -1; $text = trim((string) $update->message->text); if (!empty($parameters)) { $match = $this->matchRouteParameters($pattern, $text, $constraints); if ($match === null) return -1; $update->routeParameters = $match; return 75; } return $this->patternScore($pattern, $text, $update, $constraints);
            case 'inline_query': return $this->matchUpdateField($update, 'inline_query', 'query', $pattern, $constraints);
            case 'edited_message': return $this->matchUpdateField($update, 'edited_message', 'text', $pattern, $constraints);
            case 'channel_post': return $this->matchUpdateField($update, 'channel_post', 'text', $pattern, $constraints);
            case 'edited_channel_post': return $this->matchUpdateField($update, 'edited_channel_post', 'text', $pattern, $constraints);
            case 'chat_member': return $this->matchUpdateField($update, 'chat_member', 'new_chat_member.status', $pattern, $constraints);
            case 'my_chat_member': return $this->matchUpdateField($update, 'my_chat_member', 'new_chat_member.status', $pattern, $constraints);
            case 'chat_join_request': return $this->matchUpdateField($update, 'chat_join_request', 'invite_link.invite_link', $pattern, $constraints);
        }
        return -1;
    }

    protected function matchUpdateField(TelegramUpdate $update, string $updateKey, string $field, ?string $pattern, array $constraints): int
    {
        if (!isset($update->{$updateKey})) return -1;
        if ($pattern === null || $pattern === '') return empty($constraints) ? 100 : -1;
        $value = $this->nestedUpdateValue($update, $updateKey, $field);
        if ($value === null) return -1;
        return $this->patternScore($pattern, (string) $value, $update, $constraints);
    }

    protected function nestedUpdateValue(TelegramUpdate $update, string $root, string $path): mixed
    {
        $value = $update->{$root}; foreach (explode('.', $path) as $part) { if ($value === null || !isset($value->{$part})) return null; $value = $value->{$part}; } return $value;
    }

    protected function matchRouteParameters(?string $pattern, string $text, array $constraints): ?array
    {
        if ($pattern === null || $this->isRegex($pattern)) return null; $names = []; $compiled = ''; $offset = 0; preg_match_all('/\{([A-Za-z_][A-Za-z0-9_]*)\}/', $pattern, $matches, PREG_OFFSET_CAPTURE);
        ...
        if ($names === []) return null; $compiled .= preg_quote(substr($pattern, $offset), '/'); $result = @preg_match('/^' . $compiled . '$/u', $text, $routeMatches); if ($result !== 1 || !$this->constraintsMatch($constraints, $routeMatches)) return null; $parameters = []; foreach ($names as $name) $parameters[$name] = $routeMatches[$name]; return $parameters;
    }

    protected function patternScore(?string $pattern, string $text, TelegramUpdate $update, array $constraints = []): int
    {
        if ($pattern === null) return -1; $pattern = trim($pattern); $text = trim($text); if (!$this->isRegex($pattern)) { if ($pattern !== $text || !empty($constraints)) return -1; return 100; } $result = @preg_match($pattern, $text, $matches); if ($result === 1) { if (!$this->constraintsMatch($constraints, $matches)) return -1; $update->matches = $matches; return 50; } if ($result === false) Log::warning('Invalid Telegram route regex', ['pattern' => $pattern, 'error' => preg_last_error_msg()]); return -1;
    }

    protected function constraintsMatch(array $constraints, array $matches): bool { foreach ($constraints as $name => $expression) { if (!array_key_exists($name, $matches)) return false; if (@preg_match('/^(?:' . $expression . ')$/u', (string) $matches[$name]) !== 1) return false; } return true; }
    ...
    protected function isRegex(string $pattern): bool { if (strlen($pattern) < 3) return false; $delimiter = $pattern[0]; if (ctype_alnum($delimiter) || $delimiter === '\\') return false; $length = strlen($pattern); $escaped = false; for ($i = 1; $i < $length; $i++) { $char = $pattern[$i]; if ($escaped) { $escaped = false; continue; } if ($char === '\\') { $escaped = true; continue; } if ($char === $delimiter) { $modifiers = substr($pattern, $i + 1); return $modifiers === '' || preg_match('/^[a-zA-Z]*$/', $modifiers) === 1; } } return false; }

    protected function execute(array $route, TelegramUpdate $update): void
    {
        try {
            $middleware = TelegramBot::getGlobalMiddleware(); $configuredLimits = config('telegram-bot-router.rate_limit.limits', []); $routeLimits = $route['rate_limits'] ?? []; $limits = array_replace(is_array($configuredLimits) ? $configuredLimits : [], $routeLimits); if ((config('telegram-bot-router.rate_limit.enabled', false) || !empty($routeLimits)) && !empty($limits)) array_unshift($middleware, new \ReyhanTeam\TelegramBotRouter\Middleware\TelegramRateLimitMiddleware($limits, $route)); $middleware = array_merge($middleware, $route['middleware'] ?? []);
            $destination = function (TelegramUpdate $update) use ($route): mixed { $queueConfig = $route['queue'] ?? []; $queueEnabled = (bool) ($queueConfig['enabled'] ?? false) || (bool) config('telegram-bot-router.queue.updates', false); if (!$queueEnabled) return $this->resolveAction($route['callback'], $update); if ($route['callback'] instanceof \Closure) throw new \InvalidArgumentException('Queued Telegram routes must use a controller action or another serializable action. Closures cannot be queued.'); $job = new ProcessTelegramUpdateJob($update->originalUpdate() instanceof \stdClass ? json_decode(json_encode($update->originalUpdate()), true) : (array) $update->originalUpdate(), $route); if (!empty($queueConfig['queue'])) $job->onQueue($queueConfig['queue']); return dispatch($job); };
