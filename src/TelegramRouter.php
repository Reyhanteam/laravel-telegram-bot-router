...
class TelegramRouter
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->handleException($e, ['source' => 'router']);
        }
    ...
    public function dispatch(TelegramUpdate $update): void
    {
        ...
        if ($conversationManager->active($update)) {
            $this->execute([
                'callback' => fn (TelegramUpdate $telegramUpdate) => $conversationManager->handle($telegramUpdate),
    ...
    protected function setMatchState(array $route, TelegramUpdate $update): void
    {
        ...
        if ($type === 'callback_query' && isset($update->callback_query->data)) {
            ...
            if (!empty($route['parameters'])) {
                ...
                if ($parameters !== null) {
                    $update->routeParameters = $parameters;
                }
            }
        }
    }
    ...
    protected function patternScore(?string $pattern, string $text, TelegramUpdate $update, array $constraints = []): int
    {
        ...
        if ($result === false) {
            Log::warning('Invalid Telegram route regex', ['pattern' => $pattern, 'error' => preg_last_error_msg()]);
        }
        return -1;
    }
    ...
    protected function constraintsMatch(array $constraints, array $matches): bool
    {
        foreach ($constraints as $name => $expression) {
            ...
            if (@preg_match('/^(?:' . $expression . ')$/u', (string) $matches[$name]) !== 1) return false;
        }
        return true;
    }
    ...
    protected function execute(array $route, TelegramUpdate $update): mixed
    {
        try {
            ...
            if ((config('telegram-bot-router.rate_limit.enabled', false) || !empty($routeLimits)) && !empty($limits)) {
                array_unshift($middleware, new \ReyhanTeam\TelegramBotRouter\Middleware\TelegramRateLimitMiddleware($limits, $route));
            }

            $middleware = array_merge($middleware, $route['middleware'] ?? []);
            $destination = function (TelegramUpdate $telegramUpdate) use ($route): mixed {
                $queueConfig = $route['queue'] ?? [];
                $queueEnabled = (bool) ($queueConfig['enabled'] ?? false) || (bool) config('telegram-bot-router.queue.updates', false);

                if (!$queueEnabled) {
                    return $this->resolveAction($route['callback'], $telegramUpdate);
                }
                ...
                $job = new ProcessTelegramUpdateJob(
                    $telegramUpdate->originalUpdate() instanceof \stdClass
                        ? json_decode(json_encode($telegramUpdate->originalUpdate()), true)
                        : (array) $telegramUpdate->originalUpdate(),
                    $route,
                );
            ...
            return (new MiddlewarePipeline($middleware))->process($update, $destination);
    ...
    protected function resolveAction($action, TelegramUpdate $update): mixed
    {
        if ($action instanceof \Closure) {
            $result = $action($update, $update->commandArguments(), ...array_values($update->routeParameters));
        ...
        if (is_array($action) && count($action) === 2) {
            ...
            if (!is_string($controller) || !class_exists($controller)) {
                throw new \InvalidArgumentException("Telegram route controller [{$controller}] was not found. Check routes/bot.php.");
            }
            if (!is_string($method) || !method_exists($controller, $method)) {
                throw new \InvalidArgumentException("Telegram route method [{$method}] was not found on [{$controller}].");
            }
            ...
            return $this->resolveActionResult(app()->call([$instance, $method], $parameters));
        }

        throw new \InvalidArgumentException('Invalid Telegram route action provided.');
    }

    protected function resolveActionResult(mixed $result): mixed
    {
        if ($result instanceof TelegramResponse) {
            return $result->send();
        }
    ...
    {
        foreach (['inline_query', 'edited_message', 'channel_post', 'edited_channel_post', 'chat_member', 'my_chat_member', 'chat_join_request', 'callback_query', 'message'] as $type) {
            if (isset($update->{$type})) {
                if ($type === 'message' && isset($update->message->text)) {
                    return str_starts_with((string) $update->message->text, '/') ? 'command' : 'text';
                }
                return $type;
            }
        }
