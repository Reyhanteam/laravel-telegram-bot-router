...
class TelegramRouter
{
    ...
    public function dispatch(TelegramUpdate $update): void
    {
        ...
        if ($conversationManager->active($update)) {
            $this->execute(['callback' => fn (TelegramUpdate $update) => $conversationManager->handle($update), 'pattern' => 'conversation', 'middleware' => [], 'constraints' => []], $update); return;
        }
        ...
        if ($matchedRoute !== null) { $update->matches = $matchedMatches; $update->routeParameters = $matchedRouteParameters; $update->commandArguments = $matchedCommandArguments; $this->execute($matchedRoute, $update); return; }
        if ($fallback = TelegramBot::getFallback()) { $this->execute(['callback' => $fallback, 'pattern' => 'fallback', 'middleware' => [], 'constraints' => []], $update); return; }
        if ($onInvalid = TelegramBot::getOnInvalid()) { $this->execute(['callback' => $onInvalid, 'pattern' => 'onInvalid', 'middleware' => [], 'constraints' => []], $update); return; }
    ...
    {
        if (!isset($update->message->text)) return false; $text = trim((string) $update->message->text); if ($text === '' || !str_starts_with($text, '/')) return false;
    ...
    protected function routeMatchScore(array $route, TelegramUpdate $update): int
    {
        ...
        switch ($type) {
            case 'callback_query': if (!isset($update->callback_query)) return -1; if ($pattern === null || $pattern === '') return empty($constraints) ? 10 : -1; return $this->patternScore($pattern, (string) ($update->callback_query->data ?? ''), $update, $constraints);
            case 'command': if (!isset($update->message->text)) return -1; $text = trim((string) $update->message->text); if ($text === '' || !str_starts_with($text, '/')) return -1; $parts = preg_split('/\s+/', $text, 2); $command = $parts[0] ?? ''; $argumentText = $parts[1] ?? ''; if (str_contains($command, '@')) { $command = explode('@', $command, 2)[0]; $normalizedText = $command.($argumentText === '' ? '' : ' '.$argumentText); } else $normalizedText = $text; if (!empty($parameters)) { $match = $this->matchRouteParameters($pattern, $normalizedText, $constraints); if ($match === null) return -1; $update->routeParameters = $match; $update->commandArguments = $argumentText === '' ? [] : preg_split('/\s+/', $argumentText); return 75; } if ($command !== $pattern) return -1; $update->commandArguments = $argumentText === '' ? [] : preg_split('/\s+/', $argumentText); return empty($constraints) ? 100 : -1;
            case 'text': if (!isset($update->message->text)) return -1; $text = trim((string) $update->message->text); if (!empty($parameters)) { $match = $this->matchRouteParameters($pattern, $text, $constraints); if ($match === null) return -1; $update->routeParameters = $match; return 75; } return $this->patternScore($pattern, $text, $update, $constraints);
        }
        return -1;
    }
    ...
    protected function execute(array $route, TelegramUpdate $update): void
    {
        ...
        catch (Throwable $e) { throw new TelegramRouteException('Telegram route execution failed for ['.($route['pattern'] ?? 'fallback').']: '.$e->getMessage(), $route['pattern'] ?? null, $route['callback'] ?? null, $e); }
    }
    ...
    protected function resolveAction($action, TelegramUpdate $update)
    {
        ...
        if (is_array($action) && count($action) === 2) { [$controller, $method] = $action; if (!is_string($controller) || !class_exists($controller)) throw new \InvalidArgumentException("Telegram route controller [{$controller}] was not found. Check routes/bot.php."); if (!is_string($method) || !method_exists($controller, $method)) throw new \InvalidArgumentException("Telegram route method [{$method}] was not found on [{$controller}]."); $instance = app()->make($controller); $parameters = ['update' => $update, 'arguments' => $update->commandArguments()]; foreach ($update->routeParameters as $name => $value) $parameters[$name] = $value; return app()->call([$instance, $method], $parameters); }
        throw new \InvalidArgumentException('Invalid Telegram route action provided.');
    }

    protected function getUpdateType(TelegramUpdate $update): string { if (isset($update->callback_query)) return 'callback_query'; if (isset($update->message->text)) return str_starts_with((string) $update->message->text, '/') ? 'command' : 'text'; return 'unknown'; }
}
