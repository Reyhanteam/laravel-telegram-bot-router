...
class TelegramRouter
{
    ...
    public function dispatch(TelegramUpdate $update): void
    {
        ...
        if ($conversationManager->active($update)) {
            $this->execute(['callback' => fn(TelegramUpdate $update) => $conversationManager->handle($update), 'pattern' => 'conversation', 'middleware' => [], 'constraints' => []], $update);
            return;
        }
        $matchedRoute = null;
        $matchedScore = -1;
    ...
    protected function execute(array $route, TelegramUpdate $update): void
    {
        try {
            ...
            throw new TelegramRouteException('Telegram route execution failed for [' . ($route['pattern'] ?? 'fallback') . ']: ' . $e->getMessage(), $route['pattern'] ?? null, $route['callback'] ?? null, $e);
        }
    }
